...
class HeadNeRFNet(nn.Module):
    ...
    def calc_color_with_code(self, aud, bg_tensor, mask_tensor, fg_vps, shape_code, appea_code, FGvp_embedder, 
        ...
        # bg_featmap = self.neural_render.get_bg_featmap(bg_tensor)
        # bg_img = self.neural_render(bg_featmap, aud=None)

        merge_featmap = fg_feat #+ bg_alpha * bg_featmap # [5,256,32,32]
        merge_img = self.neural_render(merge_featmap, aud=aud)
    ...
    def _forward(
            self, 
            for_train, 
            batch_xy, batch_uv, aud, bg_tensor, mask_tensor,
        ...
        c_ori_res, batch_weight = self.calc_color_with_code(
            aud, bg_tensor, mask_tensor, fg_vps, cur_shape_code, cur_appea_code, FGvp_embedder, FGvd_embedder, FG_zdists, FG_zvals, fine_level = False
        )

...
class FittingImage(object):
    ...
    def build_info(self):
        ...
        check_dict_ours = torch.load('fitting_res/HeadNeRF__final_model_Reso32HR.pth', map_location=torch.device("cpu"))
        net = HeadNeRFNet(self.opt, include_vd=True, hier_sampling=False)        
        net.load_state_dict(check_dict_ours["headnerf"])
        self.net = net.to(self.device)
        self.net.eval()

        self.aud_dim = hparams['aud_dim']
        self.win_size = hparams['win_size']
        self.AudNet = AudioNet(self.dim_aud, self.win_size).to(self.device)
        ...
        code_dict = torch.load('fitting_res/Code__final_model_Reso32HR.pth', map_location=torch.device("cpu"))
        ...
        cam_dict = torch.load('fitting_res/Cam__final_model_Reso32HR.pth', map_location=torch.device("cpu"))
    ...
    def perform_rendering(self, img_path, mask_path, para_3dmm_path, base_name, save_root):
        ...
        for i, iter_ in enumerate(range(start, end)):
            ...
            cv2.imwrite("./fitting_res/%d.png" % iter_, coarse_fg_rgb[:, :, ::-1])

            res_img_list.append(coarse_fg_rgb)

        NVRes_save_path = "%s/Ours_%s.mp4" % (save_root, base_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--transform_json", type=str, default='/local_datasets/LipSync/cnn/0/transforms_val.json')
